...
public class Astar
{
    ...
    public Astar(AlgoHandler algo, LinkedList<Node> queue,boolean state[], int costs[], boolean Goal[])
    {
        this.algo = algo;
        ...
        while (!queue.isEmpty())  //initial check so that we know that the tree is not empty
        {
            ...
            node = getLeastCostAndHeuristic(queue,node,minCost);
    ...
    private Node getLeastCostAndHeuristic(LinkedList<Node> queue, Node node, int minCost) {
